...
const mutation = useMutation({
  mutationFn: async (updateData) => {
    try {
      const res = await axiosSecure.patch(`/rider/update-parcel/rider/int/${updateData.id}`, updateData);
      return res.data;
    } catch (error) {
      Swal.fire({
        icon: 'error',
  ...
  onSuccess: () => {
    ...
    queryClient.invalidateQueries(['RiderPickupOnline_Int', verifiedUser?.email]); 
    closeModal();
  },
  onError: () => {
    Swal.fire({
...
const handleSubmit = () => {
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold mb-4 text-center">Pickup Parcel List (International)</h1>
      <table className="table-auto w-full border-collapse border border-gray-300 shadow-lg">
        <thead>
          <tr className="bg-blue-600">
            ...
            <th className="border border-gray-300 px-4 py-2 text-white">Recipient Number</th>
            <th className="border border-gray-300 px-4 py-2 text-white">Pickup Address</th>
            <th className="border border-gray-300 px-4 py-2 text-white">Full Address</th>
  ...
  {Array.isArray(RiderPickupOnline_Int) && RiderPickupOnline_Int.length > 0 ? (
    RiderPickupOnline_Int.filter(
      ...
      <tr key={item._id} className="hover:bg-gray-100">
        ...
        <td className="border border-gray-300 px-4 py-2">{item.Sender_Name_Int}</td>
        <td className="border border-gray-300 px-4 py-2">{item.Customer_Name_Int}</td>
        <td className="border border-gray-300 px-4 py-2">{item.Customer_Contact_Number_Int}</td>
...
export default Pickup_Parcel_Int;
